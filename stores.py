...
class Weapon(object):
    ...
    def __choose_type(self, requirement=None):
        if requirement == None:
            ...
            if randint(2):
                ...
                self.Name = choice(list(
                    self.possible_melee[self.Class])).title()
            else:
                self.Class = choice(list(self.possible_ranged.keys()))
                self.Name = choice(list(
                    self.possible_ranged[self.Class])).title()
        ...
        self.Dice = str(int(self.Rarity / 2) + 1) + 'd' + str(
            choice(self.die_values[self.Class]))
    ...
    def __weigh(self, metal, cl):
        ...
        self.Weight = round(
            self.cost_and_weight[self.Class][1] * cl[metal]['Weight'] * 14, 1)
...
class Enchant(object):
    ...
    def __init__(self, iSpell=None, rechargable=True):
        ...
        if rechargable:
            self.Uses = choice([2, 4, 6, 8, 10, 12],
                               p=[.35, .3, .15, .1, .05, .05])
